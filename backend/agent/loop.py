...
async def run_agent_loop(
    ...
    try:
        ...
        session_id = log_manager.start_session(
            agent_name=f"Agent_{persona.get('name', 'General')}",
            tags=["agent_run", persona_id or "general", f"run_{run_id[:8]}"]
        )
        log_manager.log(session_id, "INFO", "STATE_CHANGE", f"Agent activated for task: {task[:200]}")
    except Exception as le:
        logger.error("Failed to start logging session in run_agent_loop: %s", le)
    ...
    try:
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if session_id:
                log_manager.log(session_id, "INFO", "STATE_CHANGE", f"Starting iteration {iteration}/{max_iterations}")
            ...
            if session_id:
                log_manager.log(session_id, "INFO", "API_CALL", f"Calling stream_chat for step {iteration}", details={"model": model, "provider": provider_name})
            ...
            if thought_tokens:
                ...
                if session_id:
                    log_manager.log(session_id, "INFO", "RESULT", f"Step {iteration} thought completed", details={"thought": thought_tokens})
            ...
            if any(sig in thought_tokens.upper() for sig in FINISH_SIGNALS):
                ...
                if session_id:
                    log_manager.log(session_id, "INFO", "DECISION", f"Task complete found at step {iteration}", details=finish_data)
            ...
            if not tc_list:
                ...
                if session_id:
                    log_manager.log(session_id, "INFO", "DECISION", f"No tool calls; completing task at step {iteration}", details=finish_data)
                    log_manager.end_session(session_id, status="completed")
                    session_id = None
                yield _sse("agent_finish", finish_data)
                return
            ...
            if session_id:
                log_manager.log(session_id, "INFO", "ACTION", f"Executing tools in parallel: {[tc['name'] for tc in tc_list]}")
            ...
            for (tc, args, call_id, icon), result in zip(parsed_calls, results):
                if result.ok:
                    ...
                    if session_id:
                        log_manager.log(session_id, "INFO", "RESULT", f"Tool execution succeeded: {tc['name']}", details=obs_data)
                    ...
                    yield _sse("agent_observe", obs_data)
                    if session_id:
                        log_manager.log(session_id, "ERROR", "ERROR", f"Tool execution failed: {tc['name']}", details=obs_data)
            ...
            if iteration % 3 == 0 and iteration < max_iterations:
                ...
                reflection_text = ""
                if session_id:
                    log_manager.log(session_id, "INFO", "ACTION", f"Performing self-reflection at step {iteration}")
                ...
                if reflection_text:
                    ...
                    if session_id:
                        log_manager.log(session_id, "INFO", "RESULT", f"Self-reflection completed: {reflection_text}")
        ...
        if session_id:
            log_manager.log(session_id, "WARNING", "DECISION", "Maximum iterations reached without finish signal", details=max_finish_data)
            log_manager.end_session(session_id, status="failed")
            session_id = None
        yield _sse("agent_finish", max_finish_data)

    except Exception as e:
        logger.error("Agent execution loop failed with exception: %s", e)
        if session_id:
            try:
                log_manager.log(session_id, "ERROR", "ERROR", f"Agent loop failed: {str(e)}")
                log_manager.end_session(session_id, status="failed")
            except Exception:
                pass
        raise
