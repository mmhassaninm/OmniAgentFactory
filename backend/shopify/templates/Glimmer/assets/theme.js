...
function normalizeArabic(s = '') {
  ...
  t = t.replace(/[٠-٩]/g, (d) => arNums.indexOf(d));
  // collapse spaces
  t = t.replace(/\s+/g, ' ');
  return t;
}
...
function PredictiveSearch(resultsContainer) {
  ...
  const configElement = document.getElementById('ps-config');
  ...
  // Function to get current locale from URL or data attribute
  function getCurrentLocale() {
    // First try to get from the locale element if it exists
    const localeElement = document.querySelector('[data-current-locale]');
    if (localeElement) {
      return localeElement.getAttribute('data-current-locale');
    }
    ...
    // Check if this is a valid locale (you might want to extend this list)
  ...
  function getSearchResults(searchTerm) {
    ...
    // Check if input looks like a SKU (alphanumeric with some common separators)
    const isSkuLike = /^[A-Za-z0-9._-]{3,}$/.test(searchTerm) && !/[\u0600-\u06FF]/.test(searchTerm);
    ...
    // Use config from the DOM
    let searchTypes = config.types;
    let fields = [...(config.fields || [])];
    
    // If SKU-like search and SKU field is enabled, prioritize products
    if (isSkuLike && fields.some(f => f.includes('variants.sku'))) {
      searchTypes = 'product';
      // Move SKU to the beginning of fields array for better matching
      fields = [
        ...fields.filter(f => f.includes('variants.sku')),
        ...fields.filter(f => !f.includes('variants.sku'))
      ];
    }
    
    // Set the search types and limit
    params.set('resources[type]', searchTypes);
    params.set('resources[limit]', config.limit);
    
    // Only include fields that are enabled in theme settings
    if (fields && fields.length > 0) {
      // Make sure to include variants.sku in the fields if it's a SKU search
      if (isSkuLike && !fields.some(f => f.includes('variants.sku'))) {
        fields.push('variants.sku');
      }
      params.set('resources[options][fields]', fields.join(','));
    }
    
    // Get current locale for the request
    const currentLocale = getCurrentLocale();
    const isRTL = currentLocale === 'ar';
    
    // Add locale and language parameters to ensure we get results in the current language
    params.set('locale', currentLocale);
    params.set('language', currentLocale);
    
    // Only show content that's been translated to the current locale
    params.set('resources[options][unavailable_products]', 'show');
    params.set('resources[options][fields_scope]', 'all');
    ...
    const makeRequest = (requestUrl, isRetry = false) => {
      return fetch(requestUrl)
        ...
        .then(text => {
          ...
          const resultsElement = doc.querySelector('#shopify-section-predictive-search');
          
          if (!resultsElement) {
            throw new Error('No results element found');
          }
          
          let resultsMarkup = resultsElement.innerHTML;
          
          // Ensure RTL is set correctly in the results
          if (currentLocale === 'ar') {
            // Add RTL direction to all text elements in the results
            resultsMarkup = resultsMarkup.replace(/<([a-z][a-z0-9]*)([^>]*)>/gi, (match, tag, attrs) => {
              // Skip self-closing and void elements
              if (match.endsWith('/>') || ['img', 'br', 'hr', 'input', 'meta', 'link'].includes(tag.toLowerCase())) {
                return match;
              }
              // Add dir="auto" to elements that contain text
    ...
    // Build the URL
    const url = `/search/suggest?${params.toString()}`;
    
    // Make the request with retry logic
    makeRequest(url)
      .catch(error => {
        // If we get a 417 and this was a SKU search, retry with all types
        if (error.status === 417 && isSkuLike && searchTypes === 'product') {
          const retryParams = new URLSearchParams(params);
          retryParams.set('resources[type]', config.types); // Use all types
          return makeRequest(`/search/suggest?${retryParams.toString()}`, true);
        }
        
        // If we still get an error, try with English locale as fallback
        if (currentLocale !== 'en') {
          const fallbackParams = new URLSearchParams(params);
          fallbackParams.set('locale', 'en');
          fallbackParams.set('language', 'en');
          // Keep the original search term when falling back to English
          fallbackParams.set('q', searchTerm);
          // Remove translation filter for fallback to show any available content
          fallbackParams.delete('resources[options][filter]');
          return makeRequest(`/en/search/suggest?${fallbackParams.toString()}`, true);
        }
        
        console.warn('Predictive search failed:', error);
      });
  }
