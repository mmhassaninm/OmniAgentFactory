...
# ── Strategies configuration endpoints ──────────────────────────────────────────
...
async def get_strategies():
    """Get all strategies, active strategy key, and strategy configurations."""
    from agent.money_agent_loop import INCOME_STRATEGIES, get_money_agent
    agent = get_money_agent()
    # Ensure settings are loaded
    await agent.load_strategy_settings()
    
    return {
        "active_strategy": agent.strategy_key,
        "strategies": INCOME_STRATEGIES
    }

@router.post("/strategy")
async def switch_strategy(req: SwitchStrategyRequest):
    """Switch the current active strategy."""
    from agent.money_agent_loop import INCOME_STRATEGIES, get_money_agent
    if req.strategy_key not in INCOME_STRATEGIES:
        raise HTTPException(status_code=400, detail=f"Invalid strategy key: {req.strategy_key}")
    ...
    try:
        ...
        if db is not None:
            await db.settings.update_one(
                {"_id": "money_settings"},
                {"$set": {"active_strategy": req.strategy_key}},
                upsert=True
            )
        
        agent = get_money_agent()
        await agent.load_strategy_settings()
        logger.info("[MoneyAPI] Switched active strategy to: %s", req.strategy_key)
        return {"status": "ok", "active_strategy": agent.strategy_key}
    except Exception as e:
        logger.error("[MoneyAPI] Failed to switch strategy: %s", e)
        raise HTTPException(status_code=500, detail=f"Failed to switch strategy: {str(e)}")

@router.post("/strategy/configure")
async def configure_strategy(req: ConfigureStrategyRequest):
    """Update settings (keywords, niche, price) for a strategy."""
    from agent.money_agent_loop import INCOME_STRATEGIES, get_money_agent
    if req.strategy_key not in INCOME_STRATEGIES:
        raise HTTPException(status_code=400, detail=f"Invalid strategy key: {req.strategy_key}")
    ...
    try:
        ...
        if db is not None:
            ...
            if req.niche is not None:
                update_fields[f"strategies_config.{req.strategy_key}.niche"] = req.niche
            if req.price is not None:
                update_fields[f"strategies_config.{req.strategy_key}.price"] = req.price
            if req.income_range is not None:
                update_fields[f"strategies_config.{req.strategy_key}.income_range"] = req.income_range
            if req.keywords is not None:
                # Filter out empty strings
                clean_keywords = [k.strip() for k in req.keywords if k.strip()]
                update_fields[f"strategies_config.{req.strategy_key}.keywords"] = clean_keywords
        ...
        agent = get_money_agent()
        await agent.load_strategy_settings()
        logger.info("[MoneyAPI] Updated configuration for strategy: %s", req.strategy_key)
        return {"status": "ok", "strategy": req.strategy_key, "config": INCOME_STRATEGIES[req.strategy_key]}
    except Exception as e:
        logger.error("[MoneyAPI] Failed to configure strategy: %s", e)
        raise HTTPException(status_code=500, detail=f"Failed to configure strategy: {str(e)}")


# ── Stealth Affiliate & Referral Marketing (Idea 48) ─────────────────────────
...
@router.get("/stealth/campaigns")
async def list_stealth_campaigns():
    """List all active passive income campaigns."""
    from agent.stealth_marketing import get_stealth_engine
    try:
        return await get_stealth_engine().list_campaigns()
    except Exception as e:
        logger.error("[MoneyAPI] Failed to list campaigns: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/stealth/campaigns")
async def create_stealth_campaign(req: CreateCampaignRequest):
    """Create a new affiliate/referral campaign."""
    from agent.stealth_marketing import get_stealth_engine
    try:
        return await get_stealth_engine().create_campaign(
            req.name, req.topic, req.referral_link, req.keywords
        )
    except Exception as e:
        logger.error("[MoneyAPI] Failed to create campaign: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.delete("/stealth/campaigns/{campaign_id}")
async def delete_stealth_campaign(campaign_id: str):
    """Delete an affiliate campaign."""
    from agent.stealth_marketing import get_stealth_engine
    try:
        success = await get_stealth_engine().delete_campaign(campaign_id)
        return {"status": "ok", "success": success}
    except Exception as e:
        logger.error("[MoneyAPI] Failed to delete campaign: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/stealth/posts")
async def list_stealth_posts(campaign_id: Optional[str] = None):
    """List generated stealth outreach posts."""
    from agent.stealth_marketing import get_stealth_engine
    try:
        return await get_stealth_engine().get_posts(campaign_id)
    except Exception as e:
        logger.error("[MoneyAPI] Failed to list posts: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/stealth/trigger/{campaign_id}")
async def trigger_stealth_outreach(campaign_id: str):
    """Trigger an active search & reply generation cycle for a campaign."""
    from agent.stealth_marketing import get_stealth_engine
    try:
        posts = await get_stealth_engine().run_outreach_cycle(campaign_id)
        return {"status": "ok", "generated_count": len(posts), "posts": posts}
    except Exception as e:
        logger.error("[MoneyAPI] Failed to run outreach cycle: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
